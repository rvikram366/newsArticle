(function(){
	...
	App.Views.AddNewsView = Backbone.View.extend({
		...
		updateArticle : function(){
			...
			this.updateModel({title: $(".title").val(),shortDescription :$(".shrtDesc").val(),
								fullDescription :$(".fullDesc").val(),
								authorName:$(".Name").val(),
								date:$(".date").val(),
								isActive:status	});	
		...
		updateModel: function(newProps) {
			
			this.model.set(_.extend({}, this.model.toJSON(), newProps));
			
		},
		
		render : function(){
			if(this.model){
					this.model.set("add",false);
					this.$el.html(this.template(this.model.toJSON()));
			}else{
	...
	$(document).ready(function(){
		...
		newsCollection.add(newsModel1);
		newsCollection.add(newsModel2);
		newsCollection.add(newsModel3);
