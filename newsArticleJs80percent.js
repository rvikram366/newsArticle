(function(){
	...
	App.Views.NewsListView = Backbone.View.extend({
		...
		/* filter : function(){
			 ...
			 this.collection.filter(function(model){					
				...
				if(_.isString(title)){
					return this.searchValue.toUpperCase() === title.toUpperCase(); 
				}					
						
			}, this)
			console.log('filtered collection: ', this.collection.toJSON());
	...
	App.Routers.ShowNewsRouter = Backbone.Router.extend({
	  ...
	  showDesc : function(id){
			...
			if(typeof showArticleView !== 'undefined' && showArticleView) {
				showArticleView.destroyView();
			}
			
			showArticleView = new App.Views.DescriptionView({
				  el : $('#newsList'),
				  collection : newsCollection,
				  article_id : id
			});		
	  ...
	  addNewArticle : function(id){
		...
		if(typeof addView !== 'undefined' && addView) {
				addView.destroyView();
			}

		addView = new App.Views.AddNewsView({
			  el : $('#newsList'),
			  article_id : id,
			  collection : newsCollection			  
		});		
